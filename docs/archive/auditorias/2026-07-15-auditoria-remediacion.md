# ARCHIVADO — Auditoría de remediación 2026-07-15

> ## ⚠️ DOCUMENTO HISTÓRICO. NO ES FUENTE DE VERDAD.
>
> Archivado el 2026-07-17. **Varios de sus hallazgos resultaron falsos** y se dejan aquí
> tal como se escribieron, sin corregir, porque el registro de cómo se llegó a una conclusión
> equivocada es el valor de este documento.
>
> Hallazgos que este archivo afirma y que son **FALSOS** (verificados contra `HEAD` el 2026-07-17):
>
> | Afirma | Realidad |
> |---|---|
> | REM-001: el compose no define nginx | `docker-compose.yml:100` sí lo define |
> | REM-008: `health` y `metrics` sin cablear | `app.module.ts:19-20,47-48`; interceptor en `:58` |
> | REM-012: "Winston sigue abierto" | `main.ts:12-17` + `app.module.ts:5,30` |
> | REM-012: `@prisma/client` sigue en `^7.2.0` | `backend/package.json` lo tiene en `^7.4.2` |
>
> **Causa raíz del error:** la auditoría original leyó el *working tree sucio* (que alguien había
> destripado) creyendo que era el estado del proyecto, en vez de leer el código commiteado.
> Esa lección está codificada en `CLAUDE.md` regla 0 (jerarquía de fuentes de verdad) y regla 1
> (auditar antes de tocar).
>
> **El trabajo pendiente vive en `docs/remediacion/PLAN-DE-REMEDIACION.md`.**

---

## Por qué se conserva

Tres cosas de valor real que no están en el historial de git:

1. **REM-005 — el intento fallido.** El primer fix del export a PDF (copiar colores con
   `getComputedStyle` en `onclone`) **no funciona**: Chrome moderno devuelve los colores en
   `oklab()`/`lab()`, no en `rgb()`, así que copia el problema en vez de resolverlo. El fix real
   fue pintar cada color en un canvas 1×1 y leer el píxel con `getImageData`.
2. **REM-002 — el bug del primitivo.** `DropdownMenuContent` tenía el `className` de un
   `DropdownMenuItem` (`flex items-center ... pl-8`), lo que apilaba los hijos en horizontal y
   colapsaba el contenido a ancho 0. Afectaba a **todo** dropdown con varios hijos.
3. **La credencial real** es `admin@clientpro.com` / `Admin123!` (los docs dicen `password123`).
